...
- Add `markupAutoSelectNextOnRemove`, set to `false` to prevent auto-selection of next markup item on removal of selected markup item.
- Add `beforeMarkupUpdateManipulatorHitbox`, receives object `{ type: 'rect', radius: 22, offset: 5, hideEdges: true|false, rect: MarkupRect }` with parameters used to update the markup shape hitboxes. Should return object for rendering the hitbox.
- Fix issue with slight jump when releasing hitbox corners.
- Add React-TypeScript preset to Doka 7.0 gold version


## 6.20.0 | 2020-12-21

- Add gold version of Doka 7.0, this version is ready for production use.
